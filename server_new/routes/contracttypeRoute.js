...
router.get("/:name", async (req, res) => {
    ...
    try {
        ...
        const contracttype = await Contracttype_DB.getContracttypeWithName(req.params.name);
        contracttype.length ? res.status(200).send(contracttype) : res.status(404).send(`Contracttype with name:${req.params.name} was not found`);
...
router.put("/:name", async (req, res) => {
    ...
    try {
        ...
        const rowsAffected = await Contracttype_DB.updateContracttypes(
            ...
            req.body.monthlyContractHours,
            req.body.maxFollowingShifts,
            req.body.maxShiftsAWeek,
            req.body.maxHoursAWeek,
            req.body.nightShiftsAllowed,
            req.body.standbyAllowed,
            req.body.maxHoursAMonth,
            req.body.maxWeekendsAYear,
            req.body.ideaalShiftsAWeek
        );
...
router.delete("/:name", async (req, res) => {
    try {
        ...
        const rowsAffected = await Contracttype_DB.deleteContracttypeWithName(req.params.name);
        rowsAffected ? res.status(204).send() : res.status(404).send(`Contracttype with name:${req.params.name} was not found`);
