...
const Verlof = ({HulpVal3, Stats}) => {
  return (
    <React.Fragment>
      ...
      <tbody>
        <tr>
          <td style={{ padding: "1px", width: "10%" }}>{Math.round(Stats.verlof.totaalUren*100)/ 100}</td>
          <td style={{ padding: "1px", width: "10%" }}>{Math.round(Stats.verlof.totaalAantalShiften * 100) / 100}</td>
          {HulpVal3.map(day =>
            <td style={{ padding: "0px", width: "10%" }}>
              <p style={{ padding: '0px', margin: '0px' }}>{Math.round(Stats.verlof[`${day}`].aantalUren * 100) / 100} uur</p>
              <p style={{ padding: '0px', margin: '0px' }}>{Stats.verlof[`${day}`].aantalShifts} shiften</p>
              <p style={{ padding: '0px', margin: '0px' }}><b>{Math.round(Stats.verlof[`${day}`].aantalUren * 10000 / (Stats.verlof.totaalUren)) / 100}</b>%</p>
            </td>
          )}
